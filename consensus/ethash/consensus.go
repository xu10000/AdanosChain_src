...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	return calcDifficultyAdanos(time, parent)
}

func calcDifficultyAdanos(time uint64, parent *types.Header) *big.Int {
	...
	theNumber := new(big.Int).Add(parent.Number, big1)
	if theNumber.Cmp(params.HardForkV1) >= 0 {
		// 8 - (block_timestamp - parent_timestamp) / 8
		x.Div(x, big.NewInt(7))
		x.Sub(big.NewInt(6), x)
	} else {
		x.Div(x, big.NewInt(8))
		x.Sub(big.NewInt(5), x)
	}
...
func (ethash *Ethash) VerifySeal(chain consensus.ChainReader, header *types.Header, posShareCheck bool, difficulty *big.Int) error {
	...
	if header.Number.Cmp(params.HardForkV1) >= 0 {
		// fmt.Println()
		// fmt.Printf("HashNoNonce    : %x     consensus.go VerifySeal\n", header.HashNoNonce().Bytes())
	...
	digest, result := myx11(digestPre, header.Nonce.Uint64(), order)
...
func AccumulateRewards(state *state.StateDB, header *types.Header, uncles []*types.Header) {
	reward := getReward(header.Number)
	state.AddBalance(header.Coinbase, reward, header.Number, header.Time)
}

func getReward(block *big.Int) *big.Int {
	var reward *big.Int

	oneyear := big.NewInt(700800)
	years := new(big.Int).Div(block, oneyear).Int64()

	if years <= 4 {
		reward = big.NewInt(4.28e+18)
	} else if years > 4 && years <= 8 {
		reward = big.NewInt(2.14e+18)
	} else {
		reward = big.NewInt(1.07e+18)
	}

	reward = new(big.Int).Mul(reward, big.NewInt(int64(100)))
